...
function [song freq] = readmidi2(midiFile)
    ...
    [otherNOTE otherENDTIME otherTempoStamp] = midiInfo(otherMIDI, freq);
    [otherTempos, otherTempo_time] = getTempoChanges(otherMIDI);
    fprintf('+------------------------------+\n');
    fprintf('|                              |\n');
...
%     tempo = otherTempos(1);
    ...
    for (track = [1 : midiTracks])
        notesForThisTrack = find(otherNOTE(:,1) == track);
        notesForthisTrack = otherNOTE(notesForThisTrack, :); % Get the notes for this track
...
%                 tempo_ind = find(otherTempo_time >= trackTime*midiTime, 1, 'first');
%                 if nnz(tempo_ind)~=0
%                     tempo = myTempo(tempo_ind);
%                 end
%                 if ~exist('oldtempo', 'var')
%                     oldtempo = 0;
%                 end
%                 if tempo~=oldtempo
%                     fprintf('tempo = %d, trackTime = %.3f\n',tempo, trackTime);
%                     oldtempo = tempo;
%                 end
%                 if length(tempo) ~= 1
%                     error('TEMPO ERROR! %d', length(tempo));
%                 end
                   
%                 currTempoTable = currTempoTable < 0;
%                 if isempty(oldTempoTable)
%                     oldTempoTable = currTempoTable;
%                 end
%                 
%                 if ~isequal(oldTempoTable, currTempoTable)
%                     tempo = myTempo(logical(currTempoTable - oldTempoTable));
%                     oldTempoTable = currTempoTable;
%                 end
...
%                             volRight = vol * trackPan(msgChannel);
                            ...
                            currTime = notesForthisTrack(counter, 5);
                            tempo_ind = find((currTime - otherTempoStamp)>0, 1, 'last');
                            if nnz(tempo_ind)~=0
                                tempo = 60./(otherTempos(tempo_ind)./1000000);
                            end
                            if ~exist('oldtempo', 'var')
                                oldtempo = 0;
                            end
...
%     fclose(fh);
% song = zeros(ceil(max(otherENDTIME).*midiTime), 1);
% 
% for i = 1: size(otherNOTE,1)
%     addSampleNew(otherNOTE(i,5), otherNOTE(i,6), otherNOTE(i, 4));
% end
    ...
    clear instr;
    songTime = floor(max(otherENDTIME)); %floor(length(song) / freq);
...
%         end
   ...
   function addSampleNew(noteStart, noteEnd, note)
        z = [];
        ...
        % Adjust midi ticks to index in sound array.
        j = round(noteStart*midiTime)+1; %floor(1 + noteStart * 60 * freq / tempo);
        k = round(noteEnd*midiTime)+1; %floor(1 + noteEnd * 60 * freq / tempo);
